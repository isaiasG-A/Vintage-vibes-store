...
function Cart({setCartId, cartId}) {
  ...
  return (
    <div>
      <Link className='btn btn-outline-primary btn-lg mb-5' to="/">Home</Link> 
      {
        savedKeys.length === 0 ? null : <button className='btn btn-outline-success btn-lg mb-5  ms-3'  onClick={placeOrder} to="/completed">Place Order</button>
      }
      {
       keys.map((num, index) => {
        const cartData = JSON.parse(localStorage.getItem(`${num}`));
        return (
          <div className="row row-cols-md-2 g-3" key={cartData.id}>
            <div className='container mb-5'>
              ...
              <img className="img-fluid pb-3 "style={{width: "75%", margin: "0 auto"}}src={cartData.image} alt="" />
                <div className="card-body">
                  ...
                  <button className="plus-minus-btns btn btn-info me-2"  onClick={() => addingItem(`${num}`, cartData)}>+</button>
                  <button className="plus-minus-btns btn btn-secondary"  onClick={() => deletingItem(num)}>-</button>
                </div>
                <button className="remove-btn btn btn-danger " onClick={() => deleteItem(num)}>Remove</button>
              </div>
            </div>
          </div>
        )
